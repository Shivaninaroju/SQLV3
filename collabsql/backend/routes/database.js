...
// Get dataset summary for analytics/visualization (tables, row counts, column types, null %)
router.get('/:databaseId/analytics-summary', authenticateToken, checkDatabaseAccess('viewer'), asyncHandler(async (req, res) => {
  const { databaseId } = req.params;
  ...
  const db = await dbManager.getUserDb(database.file_path);
  ...
  for (const table of schema.tables) {
    const rowCountRow = await dbManager.getUserRow(db, `SELECT COUNT(*) as cnt FROM "${table.name}"`);
    const total = rowCountRow?.cnt ?? 0;
    ...
    for (const col of table.columns) {
      ...
      try {
        ...
        const r = await dbManager.getUserRow(db, `SELECT COUNT(*) as total, COUNT(${esc(col.name)}) as filled FROM ${esc(table.name)}`);
        if (r && r.total > 0) {
          nullPct = Math.round((1 - r.filled / r.total) * 100);
        }
      } catch (_) {
        // column might not exist in older cache
      }
      cols.push({
        name: col.name,
        type: col.type,
        nullPct: total === 0 ? 0 : nullPct
      });
    }
