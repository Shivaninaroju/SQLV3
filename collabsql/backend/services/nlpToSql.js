...
class NLPToSQLV3 {
  ...
  classifyQuery(lowerQuery, originalQuery) {
    ...
    // General knowledge / non-SQL questions (answer conversationally, do not generate SQL)
    if (lowerQuery.match(/^(what is|what are|who is|who are|how does|how do|why is|why do|explain|define|tell me about|describe what)\s+(?!.*\b(table|column|schema|constraint|key|database|row|record|join|sql|query|select|insert|update|delete)\b)/i)) {
      return { type: 'CONVERSATIONAL', subtype: 'GENERAL_QUESTION' };
    }
    // "Explain joins" / "How does SQL work?" → conversational
    if (lowerQuery.match(/^(explain|how does|how do|what are|describe)\s+(joins?|sql|aggregat|group by|having|subquer|index|primary key|foreign key)\b/i)) {
      return { type: 'CONVERSATIONAL', subtype: 'GENERAL_QUESTION' };
    }
    ...
    if (lowerQuery.match(/\b(starts?\s+with|starting\s+with|ends?\s+with|ending\s+with|contains?|containing|beginning\s+with|like)\b/)) {
      return { type: 'DML_READ' };
    }
  ...
  // ──────────────────────────────────────────
  // TABLE CONTEXT: When selectedTable is set, reject queries targeting another table
  // ──────────────────────────────────────────
  getExplicitlyMentionedTable(userQuery, lowerQuery, schema) {
    for (const table of schema.tables) {
      const tableLower = table.name.toLowerCase();
      if (lowerQuery.match(new RegExp(`\\b(from|into|update|table)\\s+["']?${this.escapeRegex(tableLower)}["']?\\b`, 'i')))
        return table.name;
  ...
  enforceSelectedTableContext(userQuery, lowerQuery, schema, selectedTable, operationLabel) {
    if (!selectedTable) return null;
    const explicitTable = this.getExplicitlyMentionedTable(userQuery, lowerQuery, schema);
    if (explicitTable && explicitTable !== selectedTable) {
      return {
        type: 'error',
        message: `You have selected the table **${selectedTable}**. Your query targets **${explicitTable}**. Please switch to ${explicitTable} in the sidebar to run this, or rephrase to apply only to ${selectedTable}.`
      };
    }
    return null;
  }
  ...
  detectTable(userQuery, lowerQuery, schema, selectedTable = null) {
    // 0. When selectedTable is set, use it first (enforcement of "other table" is done by caller)
    if (selectedTable) {
    ...
    // 3. Detect from column names mentioned
  ...
  getFuzzyNames(colName) {
    ...
    const mappings = {
      ...
      'salary': ['salary', 'pay', 'paid', 'wage', 'compensation', 'income', 'earning'],
  ...
  handleConversational(userQuery, lowerQuery, queryClass, schema) {
    ...
    if (queryClass.subtype === 'GENERAL_QUESTION') {
      return {
        type: 'info',
        message: `That's a great question, but I'm specifically designed for database operations! Here's what I can help with:\n\n` +
          `**Query data** - "show employees where salary > 10000"\n` +
          `**Filter & search** - "find names starting with S"\n` +
          `**Aggregations** - "average salary by department"\n` +
          `**Insert records** - "insert employee name John salary 5000"\n` +
          `**Update records** - "update salary to 6000 where name is John"\n` +
          `**Delete records** - "delete employee where name is John"\n` +
          `**Schema info** - "show all tables", "show constraints"\n\n` +
          `Try asking me something about your database!`
      };
    }
  ...
  handleSelect(userQuery, lowerQuery, schema, queryClass, selectedTable) {
    ...
    const tableContextErr = this.enforceSelectedTableContext(userQuery, lowerQuery, schema, selectedTable, 'query');
    if (tableContextErr) return tableContextErr;
    const table = this.detectTable(userQuery, lowerQuery, schema, selectedTable);
  ...
  detectSelectColumns(lowerQuery, table) {
    ...
    // BUT: "show employees with salary > 10000" → * (salary is a filter, not a selection)
    const mentionedCols = [];

    // Check if column words appear as filters (with comparison/condition keywords after)
    const isFilterContext = (kw) => {
      const filterPattern = new RegExp(`\\b${kw}\\s*(>|<|>=|<=|!=|=|greater|less|above|below|between|more|over|under|is|equals?)\\b`, 'i');
      return lowerQuery.match(filterPattern);
    };

    // "names" → FIRST_NAME, LAST_NAME (but only when user explicitly asks for names)
    if (lowerQuery.match(/\bnames?\b/) && !lowerQuery.match(/\bname\s+(is|=|starts?|starting|ends?|ending|contains?|greater|less|>|<)\b/)) {
    ...
    // Check for other specific column mentions (only when they're being SELECTED, not filtered)
    const colKeywords = ['salary', 'email', 'phone', 'department', 'job', 'hire_date', 'age', 'grade', 'address'];
    for (const kw of colKeywords) {
      if (lowerQuery.includes(kw) && !isFilterContext(kw)) {
        // Only add column if it's explicitly requested (e.g., "show salary") not just mentioned in a condition
        const hasExplicitRequest = lowerQuery.match(new RegExp(`\\b(show|get|display|list|select|view)\\b.*\\b${kw}\\b`, 'i')) ||
          lowerQuery.match(new RegExp(`\\b${kw}\\b.*\\b(of|for|from)\\b`, 'i'));
  ...
  findNumericColumnFromQuery(lowerQuery, table) {
    // Look for explicitly mentioned numeric column, but skip table-name matches
    // and ID columns (prefer meaningful numeric columns like salary, age, etc.)
    const skipWords = new Set(['employee', 'employees', 'student', 'students', 'hospital', 'hospitals',
      'table', 'from', 'show', 'get', 'find', 'list', 'all', 'the', 'top', 'first', 'last',
      'highest', 'lowest', 'most', 'least', 'biggest', 'smallest']);

    // First pass: look for explicit column mention (skip ID columns and table-name-like matches)
    for (const col of table.columns) {
      if (col.primaryKey || col.name.toLowerCase().endsWith('_id')) continue;
      const fuzzy = this.getFuzzyNames(col.name);
      for (const f of fuzzy) {
        if (skipWords.has(f.toLowerCase())) continue;
    ...
    // Default to salary/amount/price-like columns, then first non-PK numeric column
  ...
  buildWhereClause(userQuery, lowerQuery, table) {
    ...
    const likePatterns = [
      { regex: /(?:names?|first_name|fname)?\s*(?:that\s+)?(?:starts?|starting|beginning)\s+with\s+["']?(\w+)["']?/i, type: 'startsWith', colHint: 'name' },
    ...
    for (const lp of likePatterns) {
      ...
      if (match) {
        ...
        if (col) {
          const upperVal = value.charAt(0).toUpperCase() + value.slice(1).toLowerCase();
          if (lp.type === 'startsWith') {
            conditions.push(`("${col.name}" LIKE '${upperVal}%' OR "${col.name}" LIKE '${value.toLowerCase()}%' OR "${col.name}" LIKE '${value.toUpperCase()}%')`);
          } else if (lp.type === 'endsWith') {
            conditions.push(`"${col.name}" LIKE '%${value}' COLLATE NOCASE`);
          } else {
            conditions.push(`"${col.name}" LIKE '%${value}%' COLLATE NOCASE`);
          }
        }
    ...
    if (conditions.length === 0) {
      const usedFuzzyNames = new Set(); // track which fuzzy names have been matched to avoid duplicates
      for (const col of table.columns) {
        ...
        for (const fName of fuzzyNames) {
          if (usedFuzzyNames.has(fName.toLowerCase())) continue;
          ...
          const patterns = [
            ...
            new RegExp(`\\bin\\s+${this.escapeRegex(fName)}\\s+['"]?([^\\s'"\\n,]+?)['"]?(?:\\s|$)`, 'i'),
            new RegExp(`\\b${this.escapeRegex(fName)}\\s+(\\d+)(?:\\s|$)`, 'i'),
          ];
          ...
          for (const pattern of patterns) {
            ...
            if (match && match[1]) {
              ...
              if (col.type.toUpperCase().includes('INT')) {
                ...
                if (!isNaN(parsed)) { conditions.push(`"${col.name}" = ${parsed}`); usedFuzzyNames.add(fName.toLowerCase()); break; }
                ...
                usedFuzzyNames.add(fName.toLowerCase());
                break;
              }
            }
          }
  ...
  handleInsert(userQuery, lowerQuery, schema, selectedTable) {
    const tableContextErr = this.enforceSelectedTableContext(userQuery, lowerQuery, schema, selectedTable, 'insert');
    if (tableContextErr) return tableContextErr;
    const table = this.detectTable(userQuery, lowerQuery, schema, selectedTable);
    if (!table) {
      return {
    ...
    // Schema mismatch: e.g. "Insert employee data into student table"
    const entityTablePairs = [
      { keyword: 'employee', tableSubstr: 'employ' },
      { keyword: 'student', tableSubstr: 'student' },
      { keyword: 'customer', tableSubstr: 'customer' },
      { keyword: 'product', tableSubstr: 'product' },
      { keyword: 'order', tableSubstr: 'order' },
      { keyword: 'department', tableSubstr: 'department' },
    ];
    const tableLower = table.name.toLowerCase();
    for (const { keyword, tableSubstr } of entityTablePairs) {
      if (lowerQuery.includes(keyword) && !tableLower.includes(tableSubstr)) {
        return {
          type: 'error',
          message: `Cannot insert ${keyword} details into ${table.name} table due to schema mismatch.`
        };
      }
    }
    ...
    // Only use explicitly mentioned columns; never map values to columns by position (avoids FIRST_NAME in LAST_NAME bugs)

    if (Object.keys(values).length === 0) {
      return {
        type: 'clarification',
        message: `Please provide values for ${table.name}.\n\nExample:\n- insert into ${table.name} name "John" salary 5000\n- insert ${table.name} ${insertableColumns.slice(0, 2).map(c => `${c.name.toLowerCase()}=value`).join(', ')}`,
        suggestions: insertableColumns.slice(0, 4).map(c => ({
          label: c.name,
          value: `Insert into ${table.name} ${c.name.toLowerCase()} "example_value"`,
          description: `Type: ${c.type}${c.notNull ? ' (Required)' : ''}`
        }))
      };
    }

    // Required columns (NOT NULL, no default) not provided → ask
    const requiredMissing = table.columns.filter(c =>
      c.notNull && !c.defaultValue && c.name !== 'id' && !(c.primaryKey && (c.name.toLowerCase().endsWith('_id') || c.name.toLowerCase() === 'id'))
    ).filter(c => values[c.name] === undefined);
    if (requiredMissing.length > 0) {
      return {
        type: 'clarification',
        message: `Required column(s) missing for ${table.name}: ${requiredMissing.map(c => c.name).join(', ')}. Please provide values for these.`,
        suggestions: requiredMissing.slice(0, 3).map(c => ({
          label: c.name,
          value: `Insert into ${table.name} ${c.name.toLowerCase()} "value"`,
          description: `${c.type} (required)`
        }))
      };
    }
  ...
  handleUpdate(userQuery, lowerQuery, schema, selectedTable) {
    const tableContextErr = this.enforceSelectedTableContext(userQuery, lowerQuery, schema, selectedTable, 'update');
    if (tableContextErr) return tableContextErr;
    const table = this.detectTable(userQuery, lowerQuery, schema, selectedTable);
    if (!table) {
      return {
    ...
    if (setStatements.length === 0) {
      return {
        type: 'clarification',
        message: `What would you like to update in ${table.name}?\n\nExample: Update ${table.name} salary to 50000 where name is John`
      };
    }

    const whereClause = this.buildWhereClause(userQuery, lowerQuery, table);
    const userSaysAll = /\b(all|every)\s+(row|record|entry|item)?s?\b/.test(lowerQuery) || /update\s+all\b/i.test(userQuery);
    if (!whereClause && !userSaysAll) {
      return {
        type: 'clarification',
        message: `UPDATE without WHERE clause will modify ALL ${table.rowCount || 'all'} rows in ${table.name}.\n\nPlease specify which rows to update, or say "update all" to confirm.\n\nExample: Update ${table.name} set column to value where id = 1`
      };
    }
  ...
  handleDelete(userQuery, lowerQuery, schema, selectedTable, conversationHistory = []) {
    const tableContextErr = this.enforceSelectedTableContext(userQuery, lowerQuery, schema, selectedTable, 'delete');
    if (tableContextErr) return tableContextErr;
    const table = this.detectTable(userQuery, lowerQuery, schema, selectedTable);
    if (!table) {
      return {
    ...
    if (!whereClause) {
      return {
        type: 'clarification',
        message: `Please specify which record(s) to delete from ${table.name}.\n\nExamples:\n- Delete from ${table.name} where name is "John"\n- Remove employee where id = 5\n- Delete from ${table.name} where salary < 3000`,
        suggestions: [
          { label: 'By name', value: `Delete from ${table.name} where name is "value"`, description: 'Delete by name match' },
          { label: 'By ID', value: `Delete from ${table.name} where id = 1`, description: 'Delete specific record' },
  ...
  extractDeleteTarget(userQuery, lowerQuery, table) {
    ...
    let cleaned = lowerQuery
      .replace(/\b(delete|remove|from|employee|student|hospital|record|row|entry|the|a|an)\b/g, '')
      .replace(new RegExp(`\\b${this.escapeRegex(tableName)}\\b`, 'g'), '')
      .trim();

    // Check for "name value" pattern
    const nameValueMatch = userQuery.match(/\b(?:name|first_name|fname)\s+["']?(\w+)["']?/i);
    if (nameValueMatch) {
    ...
    if (cleaned.length > 0) {
      ...
      if (words.length === 1) {
        // Single value - match against first text column (likely a name)
        const textCol = table.columns.find(c =>
          c.name.toLowerCase().includes('first_name') || c.name.toLowerCase() === 'name' || c.name.toLowerCase().includes('name')
        ) || table.columns.find(c => !c.type.toUpperCase().includes('INT') && !c.primaryKey);

        if (textCol) {
          // Use original case from userQuery
          const originalWord = userQuery.match(new RegExp(words[0], 'i'));
...
RULES:
1. Generate valid SQLite syntax only. Use double quotes for table/column names.
2. NEVER assume column values. Use only values the user explicitly stated. For missing optional columns use NULL.
3. NEVER put a value into the wrong column (e.g. do not put first_name value into last_name).
4. CLARIFY: [question] only when truly ambiguous (e.g. which table when multiple fit).
5. INFO: [answer] for non-SQL questions (e.g. "What is AI?", "Explain joins").
6. Limit SELECT to 100 rows unless user specifies a number.
7. Always include WHERE for UPDATE/DELETE unless user explicitly says "all" or "every".
8. Use LOWER() for case-insensitive string comparisons; use LIKE with % for pattern matching.
9. Support SELECT, INSERT, UPDATE, DELETE, CREATE TABLE, ALTER TABLE. Support JOINs, GROUP BY, HAVING, subqueries.
10. Support ORDER BY, DISTINCT, BETWEEN. Use proper types: numbers unquoted, strings in single quotes.
